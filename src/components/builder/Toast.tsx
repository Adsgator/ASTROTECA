    ...
    <ToastContext.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-4 right-4 z-[9999] flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          ...
          >
            {t.type === 'success' && <Check className="w-3.5 h-3.5 flex-shrink-0" />}
            {t.type === 'error' && <X className="w-3.5 h-3.5 flex-shrink-0" />}
            {t.type === 'info' && <Info className="w-3.5 h-3.5 flex-shrink-0" />}
